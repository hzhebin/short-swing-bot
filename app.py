...
optimize = st.sidebar.checkbox("启用策略参数优化", value=True)
...
if st.button("▶️ 开始回测"):
    for symbol in symbols:
        ...
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=df["timestamp"][:len(best["eq"])], y=best["eq"], mode="lines", name="净值"))
        for t in best["trades"]:
            color = "green" if t[2] == "buy" else "red"
            symbol_txt = "▲" if t[2] == "buy" else "▼"
            fig.add_trace(go.Scatter(
                x=[df["timestamp"].iloc[t[0]]],
                y=[t[1]],
                mode="markers+text",
                text=[symbol_txt],
        ...
        st.download_button("📥 下载回测结果 CSV", csv, file_name=f"{symbol}_results.csv", mime="text/csv")
