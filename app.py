...
if st.button("▶️ 运行策略"):
    with st.spinner("正在下载数据并回测..."):
        ...
        for i in range(1, len(df)):
            ...
            equity = balance + position * cp
            equity_curve.append(equity)

            required_margin = position * cp / leverage if position > 0 else 0
            maintenance_margin = required_margin * 0.5
            if equity <= maintenance_margin and position > 0:
                explosion += 1
                position = 0
                balance = equity
                trades.append({'step': i, 'type': '爆仓强平', 'price': cp})
                continue
        ...
        for t in trades:
            if t['type'] == 'long':
                ...
                ax.scatter(t['step'], t['price'], color='black', marker='x', label='爆仓')
        ax.set_title("交易点图")
        ax.set_xlabel("K线步数")
        ax.set_ylabel("价格")
        ax.grid(True)
