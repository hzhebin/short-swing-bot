...
end_date = st.sidebar.date_input("结束日期", value=pd.to_datetime("2025-03-30"))
...
def get_binance_kline(symbol, interval='1h', start_date='2024-04-01', end_date='2025-05-01'):
    ...
    while start_ts < end_ts:
        ...
        if not data or isinstance(data, dict):
            st.error("❌ 无法从 Binance 获取数据。请检查交易对、时间区间、或确保不包含未来日期。")
            return pd.DataFrame()
        klines.extend(data)
        start_ts = data[-1][0] + 1
        time.sleep(0.05)
